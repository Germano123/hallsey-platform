  ...
  return (
    <div className="space-y-8 font-cozy relative z-10 max-w-4xl mx-auto">
      ...
      <div>
        ...
        <p className="text-cozy-sm text-[#94a3b8] mt-1">Conecte seus cartões físicos e visualize suas perícias e atributos digitais.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-[#1c1c22] border border-white/5 border-t-4 border-t-[#fb923c] p-6 rounded-[20px] shadow-xl space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-[#121214] p-4 rounded-xl border border-white/5">
              <div>
                <span className="text-cozy-xs text-[#94a3b8] uppercase font-bold">Ordem Escolhida</span>
                <div className="text-cozy-sm font-bold text-[#34d399] mt-0.5">{favClass}</div>
              </div>
              <div className="space-y-1">
                <span className="text-cozy-xs text-[#94a3b8] uppercase font-bold block">Alterar Ordem</span>
                <div className="flex flex-wrap gap-1 text-cozy-xs">
                  {["Arquivista", "Bibliotecário", "Encadernador"].map(cls => (
                    <button 
                      key={cls}
                      onClick={() => selectClass(cls)}
                      className={`px-2 py-1 rounded text-cozy-xs font-bold border transition-all ${
            ...
            <div className="grid grid-cols-4 gap-3 text-center">
              <div className="bg-[#121214] p-3 rounded-lg border border-white/5">
                <span className="text-cozy-xs text-[#94a3b8] block">Sabedoria</span>
                <span className="text-cozy-lg font-black text-[#fb923c]">{charStats.sabedoria}</span>
              </div>
              <div className="bg-[#121214] p-3 rounded-lg border border-white/5">
                <span className="text-cozy-xs text-[#94a3b8] block">Inteligência</span>
                <span className="text-cozy-lg font-black text-[#34d399]">{charStats.inteligencia}</span>
              </div>
              <div className="bg-[#121214] p-3 rounded-lg border border-white/5">
                <span className="text-cozy-xs text-[#94a3b8] block">Força</span>
                <span className="text-cozy-lg font-black text-[#f4ebd0]">{charStats.forca}</span>
              </div>
              <div className="bg-[#121214] p-3 rounded-lg border border-white/5">
                <span className="text-cozy-xs text-[#94a3b8] block">Defesa Mágica</span>
                <span className="text-cozy-sm font-bold text-indigo-300 block truncate mt-1">{charStats.defesaMagica}</span>
              </div>
            </div>

            {/* List active spells */}
            <div className="space-y-2">
              <span className="text-cozy-xs text-[#94a3b8] uppercase font-bold tracking-wider">Feitiços Conjuráveis</span>
              <div className="flex flex-wrap gap-1.5">
                {charStats.magiasLiberadas.map((spell, idx) => (
                  <span key={idx} className="bg-indigo-950/30 border border-indigo-500/20 text-cozy-xs text-indigo-300 px-3 py-1 rounded-full">
                    ★ {spell}
                  </span>
                ))}
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#1c1c22] border border-white/5 border-t-4 border-t-[#10b981] rounded-[20px] p-6 shadow-xl space-y-4 text-center">
            <h3 className="text-base font-bold text-[#f4ebd0]">Scanner NFC Integrado</h3>
            ...
            <div className="min-h-[140px] bg-[#121214] border border-white/5 rounded-xl p-4 flex flex-col items-center justify-center relative overflow-hidden">
              {isScanning ? (
                <div className="space-y-3 flex flex-col items-center">
                  ...
                  <p className="text-cozy-xs text-[#34d399] animate-pulse">{scanningMessage}</p>
                </div>
              ) : scannedCardInfo ? (
                <div className="space-y-1 flex flex-col items-center">
                  <ShieldCheck className="w-8 h-8 text-[#34d399] mb-1" />
                  <h4 className="text-cozy-sm font-bold text-[#f4ebd0]">{scannedCardInfo.name}</h4>
                  <p className="text-cozy-xs text-[#94a3b8] leading-tight">{scannedCardInfo.desc}</p>
                  <span className="text-cozy-xs text-[#34d399] bg-[#34d399]/10 px-2 py-0.5 rounded-full mt-1 font-bold">{scannedCardInfo.change}</span>
                </div>
              ) : (
                <div className="space-y-1.5 text-[#94a3b8] flex flex-col items-center">
                  <Wifi className="w-8 h-8 animate-pulse text-[#94a3b8]/40" />
                  <p className="text-cozy-sm font-bold">Scanner Pronto</p>
                  <p className="text-cozy-xs opacity-60">Escolha um cartão virtual abaixo</p>
                </div>
              )}
            </div>

            {scannedCards.length > 0 && (
              <Button onClick={clearScans} className="w-full bg-red-950/40 hover:bg-red-900/30 border border-red-500/20 text-red-300 text-cozy-xs h-8 rounded-full">
            ...
            <div className="text-left space-y-2">
              <span className="text-cozy-xs text-[#94a3b8] uppercase font-bold">Simular Cartões Colecionáveis</span>
                  ...
                  return (
                    <div key={idx} className="p-3 bg-[#121214] border border-white/5 rounded-xl flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h4 className="text-cozy-sm font-bold text-[#f4ebd0]">{c.name}</h4>
                        </div>
                      </div>
                      <button 
                        onClick={() => simulateNfcScan(c.name, c.desc, c.change)}
                        disabled={isScanning || active}
                        className={`px-2 py-1 rounded text-cozy-xs font-bold transition-all ${
